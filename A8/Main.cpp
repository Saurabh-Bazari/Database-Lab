...
int getGap(struct Node* last, int i){
  ...
  if(i == last->data.arr.size()-1){
    gap = (DISK_PAGE_SIZE - last->data.arr[i].start - last->data.arr[i].length - last->data.arr.size()*DIR_ENTRY_LENGTH - 2*sizeof(int) - last->data.spaceLeft);
  }else{
    ...
    void DiskFile :: insertRecord(struct Node** head_ref, int rec_id, int rec_length, bool * inserted_records)
    {
        ...
        while(last != NULL) {
            ...
            if (((last->data.dirSlotCount == 1) && (last->data.arr[0].valid == false)) || (last->data.spaceLeft == (DISK_PAGE_SIZE - (sizeof(last->data.spaceLeft)+sizeof(last->data.dirSlotCount)+last->data.arr.size()*DIR_ENTRY_LENGTH)))){
                ...
                //Directory Entry Length already subtracted
            ...
            /* Case-4: When no empty slot is available and if existing Page can not accommodate the data at the end,
             ...
             /* Case-5: When no empty slot is avai0lable and no existing Page can accommodate the data,
    ...
    void DiskFile :: printPages(struct Node* node)
    {
        ...
        while (node != NULL) {
            ...
            for(int i=0; i<node->data.arr.size(); i++){
                ...
                //   gap = (DISK_PAGE_SIZE - node->data.arr[i].start - node->data.arr[i].length - node->data.arr.size()*DIR_ENTRY_LENGTH - 2*sizeof(int) - node->data.spaceLeft);
                // }else{
                //   gap = node->data.arr[i+1].start - (node->data.arr[i].start + node->data.arr[i].length);
                // }
                if(i!=node->data.arr.size()-1)
